...
class IOSVoiceToTextParser: VoiceToTextParser, ObservableObject {
    ...
    private var micObserver = MicrofonePowerObserver()
    ...
    func reset() {
        self.stopListening()
        self._state.value = VoiceToTextParserState(
            result: "",
            error: nil,
            powerRatio: 0.0,
            isSpeaking: false
    ...
    func startListening(languageCode: String) {
        ...
        let chosenLangugage = Locale.init(identifier: languageCode)
        let supportedLocale = SFSpeechRecognizer.supportedLocales().contains(chosenLangugage) ? chosenLangugage : Locale(identifier: "en-US")
        self.recognizer = SFSpeechRecognizer(locale: supportedLocale)
        
        guard recognizer?.isAvailable == true else {
            updateState(error: "Speech recognizer is not avaliable")
            return
        }
